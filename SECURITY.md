...
- **MCP CLI（`autoace-cli/`，包名 `autoace-cli`）**：在本机以用户权限运行，通过局域网 HTTP（或 `KUAIYOU_DEVICE_URL` 的 HTTPS）与 Android 设备通信。命令注入、路径穿越、不安全的临时文件处理、明文传输截屏/UI 树等属于重点防护面。当前默认通道是局域网 HTTP，请仅在可信、隔离的网络使用。
